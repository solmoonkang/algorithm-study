...
public class 데이터분석 {
     ...
     */
    public static final Map<String, Integer> columnIndices = new HashMap<>();

    static {
        columnIndices.put("code", 0);
        columnIndices.put("date", 1);
        columnIndices.put("maximum", 2);
        columnIndices.put("remain", 3);
    }
    ...
    public int[][] solution(int[][] data, String ext, int val_ext, String sort_by) {
        ...
        for (int[] row : data) {
            if (row[extIndex] < val_ext) filteredData.add(row);
        }
    ...
    public int getColumnIndex(String column) {
        return columnIndices.getOrDefault(column, -1);
    }

    public static void main(String[] args) {
        데이터분석 dataAnalysis = new 데이터분석();

        // 데이터 = [코드번호, 제조일, 최대 수량, 현재 수량]
