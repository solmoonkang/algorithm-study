...
public class 택배상자 {
     ...
     * 영재가 실어야 하는 택배상자는 크기가 모두 같으며 1번 상자부터 n번 상자까지 번호가 증가하는 순서대로
     * 컨테이너 벨트에 일렬로 놓여 영재에게 전달됩니다.
    ...
    public int solution(int[] order) {
        // TODO: 조건문 활용이 미숙했었음. 다시 한 번 풀어보자.
        Deque<Integer> sidecarContainer = new LinkedList<>();
        int index = 0;
        int containerBox = 1;
        int truckCount = 0;

        while (index < order.length) {
            if (order[index] == containerBox) {
                truckCount++;
                index++;
                containerBox++;
            } else if (!sidecarContainer.isEmpty() && sidecarContainer.peekLast() == order[index]) {
                sidecarContainer.pollLast();
                truckCount++;
                index++;
            } else {
                sidecarContainer.offerLast(containerBox);
                containerBox++;

                if (containerBox > order.length) break;
            }
        }
    ...
    public static void main(String[] args) {
        택배상자 parcelBox = new 택배상자();

        int[] firstOrder = {4, 3, 1, 2, 5};
        System.out.println("parcelBox = " + parcelBox.solution(firstOrder));

        int[] secondOrder = {5, 4, 3, 2, 1};
        System.out.println("parcelBox = " + parcelBox.solution(secondOrder));
    }
}
