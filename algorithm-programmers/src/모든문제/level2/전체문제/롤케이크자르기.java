...
public class 롤케이크자르기 {
    ...
    public int solution(int[] topping) {
        // TODO: 오른쪽 맵 업데이트 부분이 잘 이해가 되지 않음. 다시 한 번 풀어보자.
        ...
        for (int i = 0; i < topping.length - 1; i++) {
            // 왼쪽 맵 업데이트
            int leftTop = topping[i];
            leftMap.put(leftTop, leftMap.getOrDefault(leftTop, 0) + 1);

            // 오른쪽 맵 업데이트
            int rightTop = topping[i];
            if (rightMap.get(rightTop) == 1) rightMap.remove(rightTop);
            else rightMap.put(rightTop, rightMap.get(rightTop) - 1);

            if (leftMap.size() == rightMap.size()) numberOfDivide++;
        }

        return numberOfDivide;
    }

    public static void main(String[] args) {
        롤케이크자르기 cuttingRollCake = new 롤케이크자르기();

        int[] firstTopping = {1, 2, 1, 3, 1, 4, 1, 2};
        System.out.println("cuttingRollCake = " +
                cuttingRollCake.solution(firstTopping));

        int[] secondTopping = {1, 2, 3, 1, 4};
        System.out.println("cuttingRollCake = " +
                cuttingRollCake.solution(secondTopping));
    }
}
